...
class SectionSyncServiceTest extends TestCase
{
    ...
    {
        [, $student, $case] = $this->makeCase(); $id = (string) Str::uuid(); $service = app(SectionSyncService::class);
        $service->sync($case, $student, 'case_context', $id, 0, ['case_category' => 'DTP'], null, false);
    ...
    {
        [, $student, $case] = $this->makeCase(); $service = app(SectionSyncService::class);
    ...
    public function test_stale_versions_return_a_conflict_without_mutating(): void
    {
        ...
        $this->assertSame('conflict', $result['status']); $this->assertSame(409, $result['httpStatus']); $this->assertSame('first', $case->fresh()->case_category);
    }
    ...
    public function test_use_server_resolution_does_not_bump_the_version(): void
    {
        ...
        $this->assertSame('resolved_server', $result['status']); $this->assertSame(1, $case->fresh()->lock_version); $this->assertSame('server', $case->fresh()->case_category);
    }
    ...
    public function test_confirmed_replace_server_updates_and_audits(): void
    {
        ...
        $this->assertSame('resolved_replaced', $result['status']); $this->assertSame('forced', $case->fresh()->case_category); $this->assertDatabaseHas('audit_events', ['auditable_id' => $case->id, 'event_type' => 'case_context.conflict_resolved']);
    }
    ...
    {
        [, $student, $case] = $this->makeCase(); $service = app(SectionSyncService::class); $id = (string) Str::uuid();
        $service->sync($case, $student, 'case_context', $id, 0, ['case_category' => 'DTP'], null, false);
