...
class CaseVitalObservationTypeNullableMigrationTest extends TestCase
{
    use RefreshDatabase;

    private const PATH = 'database/migrations/2026_09_29_000001_make_case_vital_observation_type_nullable.php';

    public function test_an_existing_vital_row_survives_rollback_and_remigration(): void
    {
        ...
        $fresh = CaseVital::query()->withoutGlobalScopes()->find($vital->id);
        ...
        $this->assertEquals(80, $fresh->value_numeric);
        $this->assertTrue(Schema::hasColumn('case_vitals', 'observation_type'));
    }

    public function test_rollback_refuses_rather_than_silently_dropping_a_row_with_a_null_observation_type(): void
    {
        [, $student, $case] = $this->makeCase();
        $vital = CaseVital::query()->withoutGlobalScopes()->create([
            'institution_id' => $case->institution_id, 'clinical_case_id' => $case->id,
        ...
        try {
            Artisan::call('migrate:rollback', ['--path' => self::PATH]);
            $this->fail('rollback should refuse when a null observation_type row exists, not silently drop it');
        } catch (\Throwable) {
        ...
        $this->assertNotNull(CaseVital::query()->withoutGlobalScopes()->find($vital->id), 'the row must still exist after the refused rollback');
        $this->assertTrue(Schema::hasColumn('case_vitals', 'value_systolic'), 'the table must still be the post-migration schema, not a partially rebuilt one');
    }
