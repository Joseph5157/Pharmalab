...
class CaseInvestigationCreationFieldsNullableMigrationTest extends TestCase
{
    use RefreshDatabase;

    private const PATH = 'database/migrations/2026_09_29_000002_make_case_investigation_creation_fields_nullable.php';

    public function test_an_existing_investigation_row_survives_rollback_and_remigration(): void
    {
        ...
        $fresh = CaseInvestigation::query()->withoutGlobalScopes()->find($investigation->id);
        ...
        $this->assertSame('140', $fresh->result_value);
        $this->assertTrue(Schema::hasColumn('case_investigations', 'test_name'));
    }

    public function test_rollback_refuses_rather_than_silently_dropping_a_row_with_null_creation_fields(): void
    {
        [, $student, $case] = $this->makeCase();
        $investigation = CaseInvestigation::query()->withoutGlobalScopes()->create([
            'institution_id' => $case->institution_id, 'clinical_case_id' => $case->id,
        ...
        try {
            Artisan::call('migrate:rollback', ['--path' => self::PATH]);
            $this->fail('rollback should refuse when a row with null creation fields exists, not silently drop it');
        } catch (\Throwable) {
        ...
        $this->assertNotNull(CaseInvestigation::query()->withoutGlobalScopes()->find($investigation->id), 'the row must still exist after the refused rollback');
        $this->assertTrue(Schema::hasColumn('case_investigations', 'unit_not_stated'), 'the table must still be the post-migration schema, not a partially rebuilt one');
    }
