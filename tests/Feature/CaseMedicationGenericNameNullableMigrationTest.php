...
class CaseMedicationGenericNameNullableMigrationTest extends TestCase
{
    use RefreshDatabase;

    private const PATH = 'database/migrations/2026_09_29_000003_make_case_medication_generic_name_nullable.php';
    ...
    public function test_an_existing_medication_row_survives_rollback_and_remigration(): void
    {
        ...
        $medication = CaseMedication::query()->withoutGlobalScopes()->create([
            ...
            'generic_name' => 'Metformin', 'status' => 'active', 'recorded_by' => $student->id,
        ]);
        ...
        $this->assertTrue(Schema::hasColumn('case_medications', 'generic_name'));
    }

    public function test_rollback_refuses_rather_than_silently_dropping_a_row_with_a_null_generic_name(): void
    {
        [, $student, $case] = $this->makeCase();
        $medication = CaseMedication::query()->withoutGlobalScopes()->create([
            'institution_id' => $case->institution_id, 'clinical_case_id' => $case->id,
            'generic_name' => null, 'status' => 'active', 'recorded_by' => $student->id,
        ]);

        try {
            Artisan::call('migrate:rollback', ['--path' => self::PATH]);
            $this->fail('rollback should refuse when a row with a null generic_name exists, not silently drop it');
        } catch (\Throwable) {
